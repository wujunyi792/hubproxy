...
func TestRewriteAuthHeaderUsesProxyTokenRealm(t *testing.T) {
	...
	got := rewriteAuthHeader(header, "https://proxy.example", "ghcr.io")

	if !strings.Contains(got, `realm="https://proxy.example/token?ns=ghcr.io"`) {
...
func TestRewriteAuthHeaderOnlyRewritesBearerChallenge(t *testing.T) {
	...
	if !strings.Contains(got, `Bearer realm="https://proxy.example/token?ns=ghcr.io"`) {
...
func TestContainerdMirrorPrivateGHCRAuthFlow(t *testing.T) {
	...
	upstream = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/token":
			if got := r.URL.Query().Get("ns"); got != "" {
				t.Errorf("token request leaked ns query to upstream auth: %s", r.URL.RawQuery)
			}
	...
	withRegistryEndpoint(t, registryEndpoint{
		Domain:       "ghcr.io",
		Upstream:     upstream.URL,
		AuthHost:     upstream.URL + "/token",
		AuthType:     "github",
...
`)
	...
	if !strings.Contains(authHeader, `realm="https://proxy.example/token?ns=ghcr.io"`) {
		t.Fatalf("WWW-Authenticate = %q, want proxy token realm", authHeader)
	}

	w = performDockerHandlerRequest(router, http.MethodGet, "/token?ns=ghcr.io&service=ghcr.io&scope=repository%3Aowner%2Fapp%3Apull", "Basic user-pat")
	...
	w = performDockerHandlerRequest(router, http.MethodGet, "/token?ns=ghcr.io&service=ghcr.io&scope=repository%3Aowner%2Fapp%3Apull", "")
	if w.Code != http.StatusUnauthorized {
...
func TestContainerdMirrorGenericAuthServiceUsesRegistryNamespace(t *testing.T) {
	var sawTokenBasicAuth bool
	...
	upstream = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v2/team/app/manifests/latest":
			w.Header().Set("WWW-Authenticate", fmt.Sprintf(`Bearer realm="%s/auth",service="token-service",scope="repository:team/app:pull"`, upstream.URL))
			w.WriteHeader(http.StatusUnauthorized)
		case "/auth":
			if got := r.URL.Query().Get("ns"); got != "" {
				t.Errorf("token request leaked ns query to upstream auth: %s", r.URL.RawQuery)
			}
			if got := r.Header.Get("Authorization"); got == "Basic elastic-creds" {
				sawTokenBasicAuth = true
			} else {
				t.Errorf("token request Authorization = %q, want Basic elastic-creds", got)
			}
			if got := r.URL.Query().Get("service"); got != "token-service" {
				t.Errorf("token request service = %q, want token-service", got)
			}
			...
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"token":"elastic-token"}`))
		default:
			t.Errorf("unexpected upstream path: %s", r.URL.String())
			w.WriteHeader(http.StatusNotFound)
		}
...
`)
	...
	w := performDockerHandlerRequest(router, http.MethodHead, "/v2/team/app/manifests/latest?ns=docker.elastic.co", "")
	if w.Code != http.StatusUnauthorized {
		t.Fatalf("manifest status = %d, want 401; body=%s", w.Code, w.Body.String())
	}
	authHeader := w.Header().Get("WWW-Authenticate")
	if !strings.Contains(authHeader, `realm="https://proxy.example/token?ns=docker.elastic.co"`) {
		t.Fatalf("WWW-Authenticate = %q, want registry namespace in proxy token realm", authHeader)
	}
	if !strings.Contains(authHeader, `service="token-service"`) {
		t.Fatalf("WWW-Authenticate lost service parameter: %q", authHeader)
	}

	w = performDockerHandlerRequest(router, http.MethodGet, "/token?ns=docker.elastic.co&service=token-service&scope=repository%3Ateam%2Fapp%3Apull", "Basic elastic-creds")
	if w.Code != http.StatusOK {
		t.Fatalf("token status = %d, want 200; body=%s", w.Code, w.Body.String())
	}
	if strings.TrimSpace(w.Body.String()) != `{"token":"elastic-token"}` {
		t.Fatalf("token body = %q", w.Body.String())
	}
	if !sawTokenBasicAuth {
		t.Fatalf("token request with basic auth was not seen")
	}
}

func TestRegistryNamespaceAliasAndPathPrefixRouting(t *testing.T) {
	var sawDockerNamespace bool
	var sawDockerAlias bool
	var sawGHCRPrefix bool

	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("ns") != "" {
			t.Errorf("upstream request leaked ns query: %s", r.URL.RawQuery)
		}
		switch r.URL.Path {
		case "/v2/library/nginx/manifests/latest":
			if r.Header.Get("X-Test-Route") == "docker-ns" {
				sawDockerNamespace = true
			}
			if r.Header.Get("X-Test-Route") == "docker-alias" {
				sawDockerAlias = true
			}
			w.Header().Set("Content-Type", "application/vnd.oci.image.manifest.v1+json")
			w.Header().Set("Docker-Content-Digest", "sha256:docker")
			_, _ = w.Write([]byte("docker-manifest"))
		case "/v2/owner/app/manifests/tag":
			sawGHCRPrefix = true
			w.Header().Set("Content-Type", "application/vnd.oci.image.manifest.v1+json")
			w.Header().Set("Docker-Content-Digest", "sha256:ghcr")
			_, _ = w.Write([]byte("ghcr-manifest"))
		default:
			t.Errorf("unexpected upstream path: %s", r.URL.String())
			w.WriteHeader(http.StatusNotFound)
		}
	...
	withRegistryEndpoint(t, registryEndpoint{
		...
		AuthHost:     upstream.URL + "/token",
		AuthType:     "docker",
		HostAliases:  []string{"registry-1.docker.io"},
		AuthServices: []string{"registry.docker.io"},
	})
	withRegistryEndpoint(t, registryEndpoint{
		Domain:       "ghcr.io",
		Upstream:     upstream.URL,
...
`)
	...
	w := performDockerHandlerRequestWithHeaders(router, http.MethodGet, "/v2/library/nginx/manifests/latest?ns=docker.io", map[string]string{
		"X-Test-Route": "docker-ns",
	})
	if w.Code != http.StatusOK {
		t.Fatalf("docker ns status = %d, want 200; body=%s", w.Code, w.Body.String())
	}
	if got := w.Body.String(); got != "docker-manifest" {
		t.Fatalf("docker ns body = %q", got)
	}

	w = performDockerHandlerRequestWithHeaders(router, http.MethodGet, "/v2/registry-1.docker.io/library/nginx/manifests/latest", map[string]string{
		"X-Test-Route": "docker-alias",
	})
	if w.Code != http.StatusOK {
		t.Fatalf("docker alias status = %d, want 200; body=%s", w.Code, w.Body.String())
	}

	w = performDockerHandlerRequest(router, http.MethodGet, "/v2/ghcr.io/owner/app/manifests/tag", "")
	if w.Code != http.StatusOK {
		t.Fatalf("ghcr prefix status = %d, want 200; body=%s", w.Code, w.Body.String())
	}
	if got := w.Body.String(); got != "ghcr-manifest" {
		t.Fatalf("ghcr prefix body = %q", got)
	}

	if !sawDockerNamespace || !sawDockerAlias || !sawGHCRPrefix {
		t.Fatalf("routing incomplete: dockerNS=%v dockerAlias=%v ghcrPrefix=%v", sawDockerNamespace, sawDockerAlias, sawGHCRPrefix)
	}
}

func TestAnonymousRegistryChallengeIsNotRewrittenToLocalToken(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("WWW-Authenticate", `Bearer realm="https://mcr.example/token",service="mcr.microsoft.com",scope="repository:team/app:pull"`)
		w.WriteHeader(http.StatusUnauthorized)
	}))
	defer upstream.Close()
...
`)
	...
	w := performDockerHandlerRequest(router, http.MethodHead, "/v2/team/app/manifests/latest?ns=mcr.microsoft.com", "")
	if w.Code != http.StatusUnauthorized {
		t.Fatalf("manifest status = %d, want 401; body=%s", w.Code, w.Body.String())
	}
	authHeader := w.Header().Get("WWW-Authenticate")
	if strings.Contains(authHeader, "proxy.example/token") {
		t.Fatalf("anonymous registry challenge was rewritten to local token endpoint: %q", authHeader)
	}
	if !strings.Contains(authHeader, `realm="https://mcr.example/token"`) {
		t.Fatalf("anonymous registry challenge was not preserved: %q", authHeader)
	}
}
...
func TestAuthenticatedTokenRequestFailsClosedForUnknownService(t *testing.T) {
	...
	w = performDockerHandlerRequest(router, http.MethodGet, "/token?service=token-service&scope=repository%3Ateam%2Fapp%3Apull", "Basic private-creds")
	if w.Code != http.StatusBadRequest {
		t.Fatalf("generic service status = %d, want 400; body=%s", w.Code, w.Body.String())
	}
}

func TestDockerHubTokenRequestRespectsRegistryDisabled(t *testing.T) {
	loadDockerHandlerTestConfig(t, `
[registries."docker.io"]
enabled = false
`)
	...
	w := performDockerHandlerRequest(router, http.MethodGet, "/token?service=registry.docker.io&scope=repository%3Alibrary%2Fnginx%3Apull", "Basic docker-creds")

	if w.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want 400; body=%s", w.Code, w.Body.String())
	}
	if !strings.Contains(w.Body.String(), `registry "docker.io" is disabled`) {
		t.Fatalf("unexpected body: %q", w.Body.String())
	}
}

func TestDisabledRegistryRequestDoesNotFallbackToDockerHub(t *testing.T) {
	loadDockerHandlerTestConfig(t, `
[registries."ghcr.io"]
enabled = false
`)
	...
	w := performDockerHandlerRequest(router, http.MethodHead, "/v2/owner/app/manifests/latest?ns=ghcr.io", "")
	if w.Code != http.StatusForbidden {
		t.Fatalf("ns status = %d, want 403; body=%s", w.Code, w.Body.String())
	}

	w = performDockerHandlerRequest(router, http.MethodHead, "/v2/ghcr.io/owner/app/manifests/latest", "")
	if w.Code != http.StatusForbidden {
		t.Fatalf("path prefix status = %d, want 403; body=%s", w.Code, w.Body.String())
	}
}

func TestDisabledRegistryTokenRequestFailsClosedWithoutCredentials(t *testing.T) {
	loadDockerHandlerTestConfig(t, `
[registries."ghcr.io"]
enabled = false
`)
	...
	w := performDockerHandlerRequest(router, http.MethodGet, "/token?ns=ghcr.io&service=ghcr.io&scope=repository%3Aowner%2Fapp%3Apull", "")
	if w.Code != http.StatusBadRequest {
		t.Fatalf("ns token status = %d, want 400; body=%s", w.Code, w.Body.String())
	}
	if !strings.Contains(w.Body.String(), `registry "ghcr.io" is disabled`) {
		t.Fatalf("unexpected ns token body: %q", w.Body.String())
	}

	w = performDockerHandlerRequest(router, http.MethodGet, "/token?service=ghcr.io&scope=repository%3Aowner%2Fapp%3Apull", "")
	if w.Code != http.StatusBadRequest {
		t.Fatalf("service token status = %d, want 400; body=%s", w.Code, w.Body.String())
	}
}
...
func withRegistryEndpoint(t *testing.T, endpoint registryEndpoint) {
	t.Helper()

	previous, existed := registryEndpoints[endpoint.Domain]
	registryEndpoints[endpoint.Domain] = endpoint
	t.Cleanup(func() {
		if existed {
			registryEndpoints[endpoint.Domain] = previous
			return
		}
		delete(registryEndpoints, endpoint.Domain)
	})
}
